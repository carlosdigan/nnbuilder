...
export function useGraph2D({ canvasRef, canvasDimensions }: Props) {
  ...
  const getPositions = useCallback(() => {
    ...
    for (let layer = 0; layer < numLayers; layer++) {
      ...
      for (let node = 0; node < nodes.length; node++) {
        const nodePosition = getNodePosition(node, layer, nodes.length, numLayers)
        circlePositions.push(nodePosition)

        const nextLayer = layer + 1
        const nextLayerNodes = layers[nextLayer]?.nodes
        if (nextLayerNodes) {
          for (let nextLayerNode = 0; nextLayerNode < nextLayerNodes.length; nextLayerNode++) {
            const otherNodePosition = getNodePosition(nextLayerNode, nextLayer, nextLayerNodes.length, numLayers)
